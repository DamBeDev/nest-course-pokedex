...
export class PokemonService {
  ...
  async findOne(term: string) {
    ...
    if(!pokemon) throw new NotFoundException(`Pokemon with id, name or pokenumber "${term}" not found`)
  ...
  private handleExceptions(error: any) {
    if(error.code === 11000) {
      ...
      console.log(error);
      throw new InternalServerErrorException(`Can't crate/update Pokemon - Check server logs`); 
  }
}
